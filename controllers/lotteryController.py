...
class LotteryController:
    ...
    def create_lottery(self, peer_id, admin_id):
        ...
        self.lottery_data["chats"][peer_id_str] = {
            "active": True,
            "tickets": {},
            "prize_pool": 0,
            "admin": admin_id,
            "rp_mode": self.lottery_data.get("global_settings", {}).get("default_rp_mode", False),
            "ticket_price": self.lottery_data.get("global_settings", {}).get("default_ticket_price", 100),
            "winner_count": self.lottery_data.get("global_settings", {}).get("default_winner_count", 3),
    ...
    def conduct_draw(self, peer_id, admin_id):
        ...
        for place, ticket, winner_id in winners:
            ...
            response += f"💰 Приз: {prize} монет\n\n"
        
        response += f"🎟️ Всего участвовало билетов: {len(all_tickets)}\n"
        response += f"💰 Общий призовой фонд: {prize_pool} монет"
    ...
    def enable_rp_mode(self, peer_id, creator_id):
        ...
        # Инициализируем глобальные настройки если их нет
        if "global_settings" not in self.lottery_data:
            self.lottery_data["global_settings"] = {"default_rp_mode": False, "default_ticket_price": 100}
        
        self.lottery_data["global_settings"]["default_rp_mode"] = True
        self._save_lottery_data()
        
        creator_name = get_user_name(creator_id)
        return f"✅ РП-режим лотереи включен администратором {creator_name}!\n\n🎯 ИЗМЕНЕНИЯ:\n• Призовой фонд теперь рассчитывается в золотых\n• Можно настроить цену билета командой 'лотерея цена [число]'\n• По умолчанию: 1 билет = 100 золотых\n\n💡 Для отключения используйте 'лотерея рп выкл'"
    
    def disable_rp_mode(self, peer_id, creator_id):
        """Выключить РП-режим лотереи (только создатель)"""
        peer_id_str = str(peer_id)
    ...
    def set_winner_count(self, peer_id, creator_id, count):
        ...
        # Инициализируем глобальные настройки если их нет
        if "global_settings" not in self.lottery_data:
            self.lottery_data["global_settings"] = {
                "default_rp_mode": False, 
                "default_ticket_price": 100,
                "default_winner_count": 3
            }
        
        self.lottery_data["global_settings"]["default_winner_count"] = count
        self._save_lottery_data()
        
        creator_name = get_user_name(creator_id)
        return f"✅ Количество победителей изменено администратором {creator_name}!\n\n🏆 Новые настройки:\n• Количество мест: {count}\n• Следующая лотерея будет иметь {count} призовых мест\n\n💡 Изменения вступят в силу для новых лотерей"
    
    def set_ticket_price(self, peer_id, creator_id, price):
        """Установить цену билета в золотых (только создатель)"""
        peer_id_str = str(peer_id)
        ...
        self.lottery_data["global_settings"]["default_ticket_price"] = price
        self._save_lottery_data()
        
        creator_name = get_user_name(creator_id)
        rp_status = "включен" if self.lottery_data["global_settings"]["default_rp_mode"] else "выключен"
        return f"✅ Цена билета установлена администратором {creator_name}!\n\n💰 Новая цена: {price} золотых за билет\n🎯 РП-режим: {rp_status}\n\n💡 Изменения применятся к новым лотереям."
